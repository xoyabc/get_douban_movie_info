...
no_and_director_num=$(cat ${FILE} |awk -F "," 'NR>1 && $2=="否" && $3==""{num[$8]++}END{for(i in num) print num[i],i}' |sort -nr |awk '$1>1{sum+=$1}END{print sum}')
...
yes_and_director_num=$(cat ${FILE} |awk -F "," 'NR>1 && $2!="否" && $3==""{num[$8]++}END{for(i in num) print num[i],i}' |sort -nr |awk '$1>1{sum+=$1}END{print sum}')
...
no_and_ctry_num=$(cat ${FILE} |awk -F "," 'NR>1' |sort -t ',' -k5,5rn -k4,4n |awk -F "," -v dr="${no_director_name}" '$2=="否" && $3=="" && $8 !~ dr{count++} END {print count}')

echo "##### **国家/地区分组（${no_and_ctry_num}部，按数量降序）** " >> ${OUT_FILE}
cat ${FILE} |awk -F "," 'NR>1' |sort -t ',' -k5,5rn -k4,4n |awk -F "," -v dr="${no_director_name}" '$2=="否" && $3=="" && $8 !~ dr' > no_and_ctry
...
yes_and_ctry_num=$(cat ${FILE} |awk -F "," 'NR>1' |sort -t ',' -k5,5rn -k4,4n |awk -F "," -v dr="${yes_director_name}" '$2!="否" && $3=="" && $8 !~ dr{count++} END {print count}')

echo "##### **国家/地区分组（${yes_and_ctry_num}部，按数量降序）** " >> ${OUT_FILE}
cat ${FILE} |awk -F "," 'NR>1' |sort -t ',' -k5,5rn -k4,4n |awk -F "," -v dr="${yes_director_name}" '$2!="否" && $3=="" && $8 !~ dr' > yes_and_ctry
