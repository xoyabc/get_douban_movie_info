...
do
    ...
    # dou list
    curl -s "https://www.douban.com/doulist/${doulist_id}/?start=${i}&sort=time&playable=0&sub_type=" -A 'Mozilla/5.0 (Windows NT 6.1; Win64; x64)' |pup '[class="post"] a attr{href}' |awk -F "/" '{print $(NF-1)}' >> douban_subjects
    sleep 1
done

tac douban_subjects | sed -n '1,$w douban_subjects'
